        ...
        <div className="rounded-xl bg-white p-4 shadow-sm space-y-3">
            <div className="flex justify-between items-center">
                <p className="text-sm font-medium">
                    Order <span id={order._id.slice(-6)}>#{order._id.slice(-6).toUpperCase()}</span>
                </p>
            ...
            {order.paymentStatus === "completed" && action.length > 0 && (
                <div className="flex flex-wrap gap-2 pt-2">
                    {action.map((status) => (
                        <button
                            ...
                            className="rounded-lg bg-[#e32447] px-3 py-1 text-xs text-white hover:bg-[#d32f3a] disabled:opacity-50"
                        >
